...
// ─────────────────────────────────────────────────────────────
async function auditPage(url, html, status, ttfb, headers, redirectChain, imgData, brokenLinks, wpSecurity, siteBaseUrl, siteChecks = {}) {
  const issues  = [];
  ...
  if (noLazy.length > 4)
    push('Performance', 'high', `${noLazy.length} below-fold image(s) without lazy loading`,
      ...
      'Enable lazy loading: add loading="lazy" to all below-fold img tags. In cache plugins: enable "Lazy Load Images". The first hero image should use loading="eager" or fetchpriority="high" instead.');
