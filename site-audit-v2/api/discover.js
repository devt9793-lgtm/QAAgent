  ...
  while ((m = locRx.exec(xml)) !== null) {
    ...
    try {
      ...
      if (parsed.hostname === baseHostname &&
          !u.match(/\.(jpg|jpeg|png|gif|webp|pdf|zip|css|js|xml|ico|svg|mp4|mp3|woff|woff2|php)(\?|$)/i) &&
          !u.includes('/wp-admin') &&
          !u.includes('/xmlrpc') &&
          !u.includes('/wp-login') &&
          !u.includes('/feed') &&
          !u.includes('/wp-json') &&
          !u.includes('?') &&  // skip query string URLs from sitemap
          !u.endsWith('.php')) {
        urls.push(u.replace(/\/$/, '') || '/');
      }
    } catch {}
  ...
  while ((m = re.exec(html)) !== null) {
    try {
      ...
      if (abs.hostname === base.hostname) {
        ...
        if (!p.match(/\.(jpg|jpeg|png|gif|webp|pdf|zip|css|js|xml|ico|svg|mp4|mp3|woff|woff2|php|txt|log|bak|sql|gz|tar|env)$/i) &&
            !abs.pathname.includes('/wp-admin') &&
            !abs.pathname.includes('/wp-login') &&
            !abs.pathname.includes('/xmlrpc') &&
            !abs.pathname.includes('/wp-cron') &&
            !abs.pathname.includes('/wp-mail') &&
            !abs.pathname.includes('/wp-signup') &&
            !abs.pathname.includes('/feed') &&
            !abs.pathname.includes('/wp-json') &&
            !abs.pathname.includes('/trackback') &&
            !abs.pathname.includes('/comment-page') &&
            !abs.pathname.includes('/embed') &&
            !abs.pathname.endsWith('.php')) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const sitemapLocations = [
      ...
      // Yoast specific post-type sitemaps
      ...
      // All in One SEO
      origin + '/sitemap.xml',
      // News / product sitemaps
      origin + '/news-sitemap.xml',
      origin + '/product-sitemap.xml',
      origin + '/portfolio-sitemap.xml',
    ];
    ...
    // Robots + sitemap summary for display in UI
    let robotsInfo = { found: false, issues: ['robots.txt not found'], info: [], sitemaps: [] };
    const robotsCheck = await safeFetch(origin + '/robots.txt', 5000);
    if (robotsCheck && robotsCheck.status === 200) {
      const rtxt = await robotsCheck.text();
      const sitemapUrls = [];
      const srx = /Sitemap:\s*(.+)/gi;
      let sm;
      while ((sm = srx.exec(rtxt)) !== null) sitemapUrls.push(sm[1].trim());
      const issues = [];
      const info   = [];
      if (/User-agent:\s*\*/i.test(rtxt) && /Disallow:\s*\/\s*$/im.test(rtxt))
        issues.push('robots.txt is blocking ALL crawlers with Disallow: /');
      if (/User-agent:\s*Googlebot/i.test(rtxt) && /Disallow:\s*\/\s*$/im.test(rtxt))
        issues.push('robots.txt is blocking Googlebot specifically');
      if (sitemapUrls.length === 0) info.push('No Sitemap: directive in robots.txt — add one');
      robotsInfo = { found: true, issues, info, sitemaps: sitemapUrls, raw: rtxt.slice(0, 500) };
    }

    let sitemapInfo = { found: false, issues: ['No sitemap found at standard locations'], info: [], urlCount: 0 };
    for (const tried of [...fetchedSitemaps]) {
      try {
        const sr2 = await safeFetch(tried, 5000);
        if (!sr2 || sr2.status !== 200) continue;
        const xml2 = await sr2.text();
        if (!xml2.includes('<loc>')) continue;
        const urlCount = (xml2.match(/<loc>/g) || []).length;
        const info2 = [];
        const issues2 = [];
        if (!xml2.includes('<lastmod>')) info2.push('No <lastmod> dates — add for better crawl prioritisation');
        if (xml2.includes('<image:loc>')) info2.push('Image sitemap entries found ✓');
        if (urlCount > 50000) issues2.push('Sitemap exceeds 50,000 URLs — split into multiple sitemaps');
        sitemapInfo = { found: true, url: tried, urlCount, issues: issues2, info: info2 };
        break;
      } catch {}
    }
